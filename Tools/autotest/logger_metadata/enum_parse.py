...
class EnumDocco(object):
    ...
    def match_enum_line(self, line):
        ...
        m = re.match("#define\s*([A-Z0-9_a-z]+)\s+(-?\d+) *(// *(.*) *)?$", line)
        if m is not None:
    ...
    def enumerations_from_file(self, source_file):
        ...
        with open(source_file) as f:
            ...
            while True:
                ...
                if re.match("\s*//.*", line) and not "LoggerEnum" in line:
                    continue
                ...
                if state == "inside":
                    ...
                    if re.match("#else", line):
                        continue
                    if re.match(".*}\s*\w*(\s*=\s*[\w:]+)?;", line) or "@LoggerEnumEnd" in line:
