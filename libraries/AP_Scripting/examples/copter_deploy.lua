...
local deployment_alt = bind_add_param('ALT', 3, 121.9)
local DEPL_CLIMB_ALT = bind_add_param('CLIMB_ALT', 4, 150.0)
...
function update_state()
   ...
   if not reached_climb_alt then
      ...
      if altitude > DEPL_CLIMB_ALT:get() then
         ...
         gcs:send_text(MAV_SEVERITY.INFO, "DEPL: Reached climb alt")
      end
      return
   end
