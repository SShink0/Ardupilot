...
void GPIO_RPI_RP1::set_gpio_mode_in(int pin) {
    set_direction(pin, Mode::Input);
}

void GPIO_RPI_RP1::set_gpio_mode_out(int pin) {
    set_direction(pin, Mode::Output);
}

void GPIO_RPI_RP1::set_gpio_high(int pin) {
    uint32_t offset = (SYS_RIO0_OFFSET + RIO_OUT + SET_OFFSET) / REG_SIZE;
    write_register(offset, 1 << pin);
}

void GPIO_RPI_RP1::set_gpio_low(int pin) {
    uint32_t offset = (SYS_RIO0_OFFSET + RIO_OUT + CLR_OFFSET) / REG_SIZE;
    write_register(offset, 1 << pin);
}

bool GPIO_RPI_RP1::get_gpio_logic_state(int pin) {
    uint32_t offset = (SYS_RIO0_OFFSET + RIO_IN) / REG_SIZE;
