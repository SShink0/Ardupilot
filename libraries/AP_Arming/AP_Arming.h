...
public:
    ...
    // Returns the time since boot (in millis) that we armed. Returns 0 if disarmed.
    uint32_t arm_time_ms() const { return is_armed() ? last_arm_time_ms : 0; }
...
private:
    ...
    uint32_t last_arm_time_ms; // last time we successfully armed
